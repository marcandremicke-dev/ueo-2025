...
export const handler = async (event) => {
  try {
    ...
    if (!id || typeof index !== 'number') {
      return { statusCode: 400, body: JSON.stringify({ error: 'Missing id/index' }) };
    }

    const n1 = Number(g1), n2 = Number(g2);
    if (![0,1,2,3,4].includes(n1) || ![0,1,2,3,4].includes(n2)) {
      return { statusCode: 400, body: JSON.stringify({ error: 'Scores must be 0-4' }) };
    }

    // Kein Unentschieden: exakt ein 4er, der andere 0..3
    const valid = ((n1 === 4 && n2 >= 0 && n2 <= 3) || (n2 === 4 && n1 >= 0 && n1 <= 3)) && !(n1 === 4 && n2 === 4);
    if (!valid) {
      return { statusCode: 400, body: JSON.stringify({ error: 'Invalid Fast4 result (needs 4:x, x<=3)' }) };
    }
    ...
    return { status    return { statusCode: 500, body: JSON.stringify({ error: 'Server error', detail: String(e) }) };
  }
